...
const SUGGESTED_QUESTIONS = [
  ...
  "¿Qué ayuda recibo en Starter, Pro y Enterprise?",
...
export function PublicAiFaq() {
  ...
  return (
    <section className="card-glass p-6 md:p-8 space-y-5">
      <div>
        <div className="text-xs uppercase tracking-widest text-primary font-bold">Preguntas antes de comprar</div>
        <h3 className="text-2xl md:text-3xl font-black mt-1">Haz una pregunta y mira cómo te responde la ayuda de la app.</h3>
        <p className="text-muted-foreground mt-2 max-w-3xl">
          Aquí ves cómo responde la app cuando un cliente quiere entender si le sirve para ahorrar plata, dejar de recomprar lento y comprar con más cabeza.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3 md:flex-row">
        <textarea
          ...
          placeholder="Escribe la pregunta que te haría un ferretero antes de empezar la prueba..."
          value={question}
          onChange={(e) => setQuestion(e.target.value)}
        />
        <Button className="md:self-end" size="lg" onClick={() => askQuestion(question)} disabled={loading || !question.trim()}>
          {loading ? "Respondiendo..." : "Ver respuesta"}
        </Button>
      </div>
      ...
      <div className="rounded-2xl border border-primary/20 bg-primary/10 p-5 min-h-28">
        ...
        <p className="text-sm leading-7 text-foreground whitespace-pre-wrap">
          {answer || "Haz una pregunta frecuente y te mostramos cómo vender OptiFerre sin palabras raras."}
        </p>
      </div>
    </section>
  );
